...
class Student {
  ...
  static async all() {
    let result = await db.query(`
      SELECT 
        ...
        sp.name AS "species"
      FROM students s
      JOIN species sp ON sp.id = species_id
      ORDER BY id`);
  ...
  static async get(id) {
    ...
    if (!result.rows.length) {
      const err = new Error(`No such student: ${id}`);
      err.status = 404;
      throw err;
    }
  ...
  static async update(id, data) {
    ...
    const result = await db.query(
      `UPDATE students 
        SET password=$2, first_name=$3, middle_name=$4, last_name=$5
        WHERE id=$1
        RETURNING id, first_name, middle_name, last_name, species_id`,
      [id, data.password, data.first_name, data.middle_name, data.last_name]
    );
